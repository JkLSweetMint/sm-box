...
const (
	loggerInitiator = "transport-[servers]-[grpc]=url_shortner-service"
)

// Server - описание grpc сервера для сервиса управления короткими url.
...
func New(ctx context.Context) (srv Server, err error) {
	...
	// Контроллеры
	{
		...
		// UrlsManagement
		{
			if ref.controllers.UrlsManagement, err = urls_management_adapter.New_Grpc(ref.ctx); err != nil {
				return
			}
		}
	}
	...
	// grpc
	{
		...
		pb.RegisterUrlShortnerServiceServer(ref.grpc, ref)
	}

	ref.components.Logger.Info().
		Text("The grpc server url shortner service has been created. ").
