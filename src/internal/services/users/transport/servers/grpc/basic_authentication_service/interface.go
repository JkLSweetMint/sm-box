...
import (
	...
	basic_authentication_adapter "sm-box/internal/services/users/infrastructure/adapters/basic_authentication"
...
func New(ctx context.Context) (srv Server, err error) {
	...
	// Контроллеры
	{
		...
		// BasicAuthentication
		{
			if ref.controllers.BasicAuthentication, err = basic_authentication_adapter.New_Grpc(ref.ctx); err != nil {
				return
			}
		}
	}
