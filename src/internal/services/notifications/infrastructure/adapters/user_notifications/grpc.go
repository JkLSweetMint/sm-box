...
const (
	loggerInitiator_Grpc = "infrastructure-[adapters]=user_notifications-(Grpc)"
)
...
func New_Grpc(ctx context.Context) (adapter *Adapter_Grpc, err error) {
	...
	adapter.components.Logger.Info().
		Format("A '%s' adapter for Grpc has been created. ", "user_notifications").Write()

	return
}

// CreateOne - создание пользовательского уведомления.
func (adapter *Adapter_Grpc) CreateOne(ctx context.Context, constructor *constructors.UserNotification) (notification *models.UserNotificationInfo, cErr c_errors.Grpc) {
	// tracer
	{
...
func (adapter *Adapter_Grpc) Create(ctx context.Context, constructors ...*constructors.UserNotification) (notifications []*models.UserNotificationInfo, cErr c_errors.Grpc) {
	// tracer
	{
		var trc = tracer.New(tracer.LevelAdapter)
