...
const (
	loggerInitiator = "transport-[servers]-[grpc]=popup_notifications-service"
)

// Server - описание grpc сервера для сервиса управления пользовательскими уведомлениями.
...
func New(ctx context.Context) (srv Server, err error) {
	...
	// Контроллеры
	{
		...
		// PopupNotifications
		{
			if ref.controllers.PopupNotifications, err = popup_notifications_adapter.New_Grpc(ref.ctx); err != nil {
				return
			}
		}
	}
	...
	// grpc
	{
		...
		pb.RegisterPopupNotificationsServiceServer(ref.grpc, ref)
	}

	ref.components.Logger.Info().
		Text("The grpc server popup notifications service has been created. ").
