...
func (srv *service) serve(ctx context.Context) (err error) {
	...
	// Транспортная часть
	{
		...
		go func() {
			...
			if err = srv.Transport().Servers().GrpcAuthenticationService().Listen(); err != nil {
				srv.Components().Logger().Error().
					Format("Failed to launch 'grpc server authentication service': '%s'. ", err).Write()
			}
		}()
	}
...
func (srv *service) shutdown(ctx context.Context) (err error) {
	...
	// Транспортная часть
	{
		...
		if err = srv.Transport().Servers().GrpcAuthenticationService().Shutdown(); err != nil {
			srv.Components().Logger().Error().
				Format("Failed to stop 'grpc server authentication service': '%s'. ", err).Write()
		}
	}
